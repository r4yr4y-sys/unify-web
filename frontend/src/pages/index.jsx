...
function StudyRing({ label, timeLabel, progress }) {
  const radius = 112;
  ...
  return (
    <div className="study-timer-ring" aria-live="polite">
      <svg className="study-timer-ring__svg" viewBox="0 0 260 260" aria-hidden="true">
        <circle
...
function StudyTimerPreview({ background, selected, onSelect }) {
  return (
    <button
      type="button"
    ...
    >
      ...
      <span className={`study-timer-background__accent study-timer-background__accent--${background.tint}`} />
...
function StudyModeButton({ mode, selected, onSelect }) {
  return (
    <button
      type="button"
...
export function StudyPage() {
  ...
  useEffect(() => {
    if (!["active", "paused", "break"].includes(timerStage)) return undefined;
    const timer = window.setInterval(() => setNow(Date.now()), 250);
    return () => window.clearInterval(timer);
  }, [timerStage]);

  const activeBackground =
    studyTimerBackgrounds.find(
      (background) =>
        background.id === (session?.backgroundId || selectedBackgroundId),
    ) ||
    studyTimerBackgrounds[0];

  const sessionReferenceTime = session?.pauseStartedAt ?? now;
  const elapsedMs = session
    ? Math.max(0, sessionReferenceTime - session.startedAt - session.pausedMs)
    : 0;
  const remainingMs =
  ...
  const breakRemainingMs =
    timerStage === "break" && session?.breakEndsAt
      ? Math.max(0, session.breakEndsAt - now)
      : 0;
  const isCountdownComplete =
    timerStage === "active" && session?.mode === "countdown" && remainingMs <= 0;
  const timerLabel =
    session?.mode === "countdown"
      ? formatStudyClock(remainingMs / 1000)
      : formatStudyClock(elapsedMs / 1000);
  const timerProgress =
    session?.mode === "countdown" && session.durationMs > 0
      ? Math.min(1, elapsedMs / session.durationMs)
      : 0;
  const breakLabel =
    breakRemainingMs > 0 ? formatStudyClock(breakRemainingMs / 1000) : "00:00";

  useEffect(() => {
    if (!isCountdownComplete || !session) return;
  ...
  }, [isCountdownComplete, session]);

  useEffect(() => {
    if (timerStage !== "break" || !session?.breakEndsAt) return;
    if (now < session.breakEndsAt) return;
    setSession((current) => {
    ...
    setTimerStage("active");
  }, [now, session?.breakEndsAt, timerStage]);
  ...
  function closeTimerSetup() {
    setTimerOpen(false);
    setTimerError("");
    if (timerStage === "setup") setTimerStage("idle");
  }
  ...
  function startStudySession() {
    ...
    const durationMs =
      timerMode === "countdown" ? (hours * 60 + minutes) * 60 * 1000 : 0;

    if (timerMode === "countdown" && (!Number.isFinite(durationMs) || durationMs <= 0)) {
      setTimerError("Choose a study duration before starting.");
      return;
    }
  ...
  function pauseStudySession() {
    if (timerStage !== "active" || !session) return;
    setSession((current) =>
  ...
  function resumeStudySession() {
    if (timerStage !== "paused" || !session) return;
    setSession((current) => {
  ...
  function endBreakEarly() {
    if (timerStage !== "break" || !session) return;
    setSession((current) => {
      if (!current) return current;
      const pausedAt = current.pauseStartedAt ?? Date.now();
      return {
  ...
  return (
    <section className="page study-page study-overview">
      ...
      <div className="study-overview__grid">
        {sections.map(({ title, copy, stat, to, icon: Icon, hue, onClick, button }) => {
          const className = `study-summary-card study-summary-card--${hue}${button ? " study-summary-card--button" : ""}`;
          const content = (
            <>
              <div>
      ...
      {timerOpen && timerStage === "setup" && (
        <div className="grades-modal-backdrop study-timer-backdrop" role="presentation" onClick={closeTimerSetup}>
          <section
          ...
          >
            ...
            <div className="study-timer-modal__grid">
              <section className="study-timer-panel">
                ...
                <div className="study-timer-mode-list">
                  {studyTimerModes.map((mode) => (
                    <StudyModeButton
                      key={mode.id}
                      mode={mode}
                      selected={timerMode === mode.id}
                ...
                {timerMode === "countdown" ? (
                  <div className="study-timer-duration">
                    ...
                    <div className="study-timer-duration__preview">
                      ...
                      <strong>{formatStudyClock((((Number(studyHours) || 0) * 60 + (Number(studyMinutes) || 0)) * 60))}</strong>
                    </div>
                  </div>
                ) : (
                ...
                {timerError && <p className="study-timer-error">{timerError}</p>}
              </section>
              <section className="study-timer-panel">
                <div className="study-timer-panel__heading">
                  <h3>Background</h3>
                ...
                <div className="study-timer-backgrounds">
                  {studyTimerBackgrounds.map((background) => (
                    <StudyTimerPreview
                      key={background.id}
            ...
            <div className="study-timer-modal__footer">
              <Button variant="secondary" type="button" onClick={closeTimerSetup}>
                Cancel
              </Button>
      ...
      {session && timerStage !== "idle" && timerStage !== "setup" && (
          ...
          <div className="study-timer-overlay__content">
            <div className="study-timer-overlay__top">
              ...
              <button
                ...
                onClick={timerStage === "finished" ? closeExperience : finishStudySession}
              >
                {timerStage === "finished" ? "Close" : "Finish"}
              </button>
            </div>
            ...
            {timerStage === "finished" ? (
              <section className="study-timer-finished">
                ...
                <p>
                  The timer is ready for another session whenever you are.
                </p>
                <div className="study-timer-actions">
                  <Button type="button" onClick={restartStudySession}>
                    Start another session
                  </Button>
                  <Button type="button" variant="secondary" onClick={closeExperience}>
                    Back to Study
                  </Button>
                </div>
              </section>
              ...
              <section className="study-timer-break">
                ...
                <h2>{session.breakLabel || "Break"}</h2>
                <strong>{breakLabel}</strong>
                <p>
                  Take a short reset. Your study timer will resume automatically
                  when the break ends.
                </p>
                <div className="study-timer-actions">
                  <Button type="button" variant="secondary" onClick={endBreakEarly}>
                    End break
                  </Button>
                  <Button type="button" variant="secondary" onClick={finishStudySession}>
                    Finish session
                  </Button>
                </div>
              </section>
              ...
              <section className="study-timer-open-display">
                ...
                <p>
                  No maximum duration, no progress percentage. Keep your focus as
                  long as you need.
                </p>
              </section>
            )}
            ...
            {timerStage === "active" && (
              <div className="study-timer-actions">
                <Button type="button" variant="secondary" onClick={pauseStudySession}>
                  Pause
                </Button>
                {breakOptions.map((option) => (
                  <Button
                    ...
                    variant="secondary"
                    key={option.id}
                    onClick={() => startBreak(option.durationMs, option.label)}
                  >
                    {option.label}
                  </Button>
                ))}
                <Button type="button" onClick={finishStudySession}>
                  Finish
                </Button>
              </div>
            )}
            ...
            {timerStage === "paused" && (
              <div className="study-timer-actions">
                <Button type="button" onClick={resumeStudySession}>
                  Resume
                </Button>
                <Button type="button" variant="secondary" onClick={finishStudySession}>
                  Finish
            ...
            {timerStage === "paused" && (
              <p className="study-timer-note">
                The timer is paused and will continue from the exact same point.
              </p>
            )}
          </div>
        </div>

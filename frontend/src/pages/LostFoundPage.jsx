...
import { Check, CheckCircle2, Mail, MapPin, PackageOpen, Pencil, Phone, Plus, Search, SearchCheck, Ticket, Trash2, X } from "lucide-react";
...
function ReportItemModal({ item, onClose, onSave, saving }) {
  const [values, setValues] = useState(() => item ? { status: item.status, title: item.title, location: item.location, description: item.description || "", contactEmail: item.contactEmail, contactPhone: item.contactPhone } : { status: "Lost", title: "", location: "", description: "", contactEmail: "", contactPhone: "" });
  ...
  const submit = (event) => {
    ...
    if (!values.title.trim() || !values.location.trim() || !values.contactEmail.trim() || !values.contactPhone.trim()) return setError("Item name, location, email, and mobile number are required.");
  ...
  return <div className="lost-report-backdrop" role="presentation" onClick={onClose}><section className="lost-report-modal" role="dialog" aria-modal="true" aria-labelledby="report-item-title" onClick={(event) => event.stopPropagation()}>
    ...
    <form onSubmit={submit} className="lost-report-form"><header><p className="eyebrow">Lost & found</p><h2 id="report-item-title">{item ? "Edit report" : "Report an item"}</h2><p>Share enough detail for other students to recognize and contact you about the item.</p></header>
      ...
      <div className="lost-report-form__contact"><label><span>Email address</span><input type="email" maxLength="254" value={values.contactEmail} onChange={change("contactEmail")} placeholder="you@example.com" /></label><label><span>Mobile number</span><input type="tel" maxLength="30" value={values.contactPhone} onChange={change("contactPhone")} placeholder="e.g. 01XXXXXXXXX" /></label></div>
...
export default function LostFoundPage() {
  const [mode, setMode] = useState("All"), [query, setQuery] = useState(""), [items, setItems] = useState([]), [contactDetails, setContactDetails] = useState({}), [editingItem, setEditingItem] = useState(null), [showReport, setShowReport] = useState(false), [loading, setLoading] = useState(true), [saving, setSaving] = useState(false), [error, setError] = useState("");
  ...
  async function saveItem(values) {
    ...
    try { const isEditing = Boolean(editingItem); const response = await fetch(`${apiUrl}/api/lost-found-items${isEditing ? `/${editingItem.id}` : ""}`, { method: isEditing ? "PUT" : "POST", headers: authHeaders(), body: JSON.stringify(values) }); const result = await response.json(); if (!response.ok) throw new Error(result.message || "Unable to save your item report."); setItems((current) => isEditing ? current.map((item) => item.id === result.item.id ? result.item : item) : [result.item, ...current]); setShowReport(false); setEditingItem(null); } catch (requestError) { setError(requestError.message); } finally { setSaving(false); }
  }
  ...
  const visibleItems = items.filter((item) => {
    const matchesMode = mode === "All" || item.status === mode;
  ...
  return <section className="page campus-page"><PageHeader eyebrow="Campus life" title="Lost & found" description="A little help to reconnect students with the things they care about." actions={<Button onClick={() => { setEditingItem(null); setShowReport(true); }}><Plus size={17} /> Report an item</Button>} />
    ...
    <div className="campus-toolbar"><div className="filter-pills">{["All", "Lost", "Found"].map((filter) => <button className={`filter-pill ${mode === filter ? "is-active" : ""}`} type="button" key={filter} onClick={() => setMode(filter)}>{filter} items</button>)}</div></div>
    ...
    {error && <p className="lost-page-error" role="alert">{error}</p>}{loading ? <p className="lost-page-empty">Loading item reports…</p> : visibleItems.length === 0 ? <p className="lost-page-empty">No matching {mode === "All" ? "" : mode.toLowerCase()} item reports yet.</p> : <div className="lost-items">{visibleItems.map((item) => <article className="lost-item" key={item.id}><ItemArtwork label={item.status === "Lost" ? "LOST" : "FOUND"} hue={item.status === "Lost" ? "amber" : "green"} /><div className="lost-item__content"><div className="lost-item__top"><span className={`status status--${item.status.toLowerCase()}`}>{item.status}</span><span>{formatDate(item.createdAt)}</span></div><h2>{item.title}</h2><p><MapPin size={15} /> {item.location}</p>{item.description && <p className="lost-item__description">{item.description}</p>}{contactDetails[item.id] && <div className="lost-contact"><strong>Contact the reporter</strong><a href={`mailto:${contactDetails[item.id].contactEmail}`}><Mail size={14} /> {contactDetails[item.id].contactEmail}</a><a href={`tel:${contactDetails[item.id].contactPhone}`}><Phone size={14} /> {contactDetails[item.id].contactPhone}</a></div>}</div>{item.canEdit ? <div className="lost-item__owner-actions"><button type="button" onClick={() => { setEditingItem(item); setShowReport(true); }}><Pencil size={15} /> Edit</button><button type="button" className="is-delete" onClick={() => deleteItem(item)}><Trash2 size={15} /> Delete</button></div> : <button type="button" className={contactDetails[item.id] ? "claim-button is-claimed" : "claim-button"} onClick={() => toggleContact(item)}>{contactDetails[item.id] ? <><Check size={16} /> Contact details shown</> : item.status === "Found" ? "This is mine" : "I found it"}</button>}</article>)}</div>}
